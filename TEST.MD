...
```
Name: Sabarikarthika S
Roll No: 24CC044
Date: 19-12-2025

Section A - MCQ Answers:
Q1: b) Parses incoming JSON request bodies
Q2: b) Salt rounds
Q3: c) Stateless authentication
Q4: b) Structure of documents in MongoDB collection
Q5: b) 201
Q6: b) Cross-Origin Resource Sharing
Q7: c) URL route parameters
Q8: b) Security headers
Q9: b) Loading environment variables from .env file
Q10: b) Only on mount (once)

Section A - Short Answers:
Q11: req.params → Data from URL
Example: /users/5 → req.params.id = 5

req.query → Data after ? in URL
Example: /products?page=2 → req.query.page = 2

req.body → Data sent in POST/PUT request
Example: { email: "test@gmail.com" }

Q12:Password hashing protects user passwords.
If the database is hacked, hashed passwords cannot be easily converted back to original passwords.
Storing plain passwords is unsafe.

Q13:Rate limiting restricts the number of requests a user can make in a given time.
It helps prevent brute-force attacks, spam, and server overload.

Q14:async/await allows handling asynchronous database calls in a readable way.
It waits for database results before executing the next line of code.

Q15: findOne() → returns one document

find() → returns multiple documents as an array
